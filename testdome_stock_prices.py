...
def most_corr(prices):
    ...
    """
    change = prices.pct_change().dropna()  # Compute percentage change and remove NaN rows
    corr = change.corr(method='pearson')
    np.fill_diagonal(corr.values, 0)
    max_corr_index = np.unravel_index(np.argmax(corr.values), corr.shape)  # Find indices of max correlation
    tickers = corr.columns
    print(tickers)
    return (tickers[max_corr_index[0]], tickers[max_corr_index[1]])
